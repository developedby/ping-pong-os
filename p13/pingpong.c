...
int sem_create (semaphore_t *s, int value)
{
  ...
  // Não é seguro, já que s->state pode ser CREATED pq *s não foi inicializado
  if (s->state == CREATED)
  {
    return -1;
  }
...
int barrier_create (barrier_t *b, int N)
{
  ...
  b->count = N;
  b->max_count = N;
  b->state = CREATED; // Melhor fazer por ultimo pra evitar problemas de concorrencia
  return 0;
}
...
int mqueue_send (mqueue_t *queue, void *msg)
{
  ...
  if (sem_down(&(queue->send_sem)) == 0)
  {
    ...
    memcpy(new_elem->msg, msg, queue->msg_size);
    new_elem->next = NULL;
    new_elem->prev = NULL;
